...
const string Evaluator::OPERATOR_PRECEDENCE[NUMBER_OF_PRECEDENCES] = { "({[","}])","||","&&","==!=",">,>=,<,<=","+-", "*/%", "^", "m++--!" };
...
int Evaluator::eval(const string& expression)
{
    ...
    //        }
			...
			if (!operators.empty() && !(the_operator =="("))
			{
...
void Evaluator::solve(string current_operator)
{
	...
	while (!operators.empty() && !is_greater_precedence(current_operator))
	{
        ...
        if (OPERATOR_PRECEDENCE[0].find(the_operator) != -1)
            break;
        if (OPERATOR_PRECEDENCE[1].find(the_operator) != -1)
            continue;
